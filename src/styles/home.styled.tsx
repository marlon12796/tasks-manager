...
export const TasksCount = styled.div<{ glow: boolean }>`
  ...
  width: 650px;
  border: 1px solid
    ${({ theme }) => (getFontColor(theme.secondary) === ColorPalette.fontLight ? '#44479cb7' : theme.primary)};
...
export const ProgressPercentageContainer = styled(Box)<{ glow: boolean }>`
  ...
  border: 1px solid
    ${({ theme }) => (getFontColor(theme.secondary) === ColorPalette.fontLight ? '#44479cb7' : theme.primary)};
  box-shadow: ${({ theme }) =>
    `0 0 18px -2px ${getFontColor(theme.secondary) === ColorPalette.fontLight ? '#090b22' : '#bababa'}`};
  & .MuiTypography-root {
    color: ${({ theme }) => getFontColor(theme.secondary)};
  }
