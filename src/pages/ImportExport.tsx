import { FileDownload, FileUpload, IntegrationInstructionsRounded, Link } from '@mui/icons-material'
...
const ImportExport = () => {
  ...
  const [selectedTasks, setSelectedTasks] = useStorageState<UUID[]>([], 'tasksToExport', 'sessionStorage') // Array of selected task IDs
  ...
  const handleImport = (taskFile: File) => {
    ...
    if (file) {
      if (file.type !== 'application/json') {
        showToast(
          <div>
            Incorrect file type {file.type !== '' && <span translate='no'>{file.type}</span>}. Please select a JSON file.
          </div>,
          { type: 'error' }
        )
        return
      ...
      reader.onload = (e: ProgressEvent<FileReader>) => {
        try {
          ...
          if (!Array.isArray(importedTasks)) {
            showToast('Imported file has an invalid structure.', {
              type: 'error'
            })
            return
          }
          ...
          if (invalidTasks.length > 0) {
            ...
            console.error(`These tasks cannot be imported due to exceeding maximum character lengths: ${invalidTaskNames}`)
            showToast(`These tasks cannot be imported due to exceeding maximum character lengths: ${invalidTaskNames}`, {
              type: 'error'
            })
            return
          }
          ...
          const isCategoryColorValid = (category: Category) => category.color && isHexColor(category.color)
          ...
          if (file.size > maxFileSize) {
            showToast(`File size is too large (${file.size}/${maxFileSize})`, {
              type: 'error'
            })
            return
          }
  ...
  const handleImportFromLink = async (): Promise<void> => {
    try {
      ...
      if (text.startsWith(`${location.protocol}//${location.hostname}`)) {
        ...
        showToast(<div>Failed to import task from the provided link. Please ensure that the link is copied correctly.</div>, {
          type: 'error'
        })
      }
    } catch (err) {
  ...
  return (
    <>
      <TopBar title='Transferir Tareas' />
      <ManagementHeader>
        Selecciona tareas para exportar &nbsp;
        <Tooltip title='Duplicados seran removidos durante el import '>
      ...
      <ManagementButtonsContainer>
        ...
        >
          <ManagementButton onClick={handleExport} disabled={selectedTasks.length === 0}>
            <FileDownload /> &nbsp; Exportar seleccionadas a JSON {selectedTasks.length > 0 && `[${selectedTasks.length}]`}
          </ManagementButton>
        </Tooltip>
