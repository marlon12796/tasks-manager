...
const NotFound: React.FC<NotFoundProps> = ({ message }) => {
  ...
  useEffect(() => {
    document.title = 'Aplicación de Tareas - Página no encontrada'
  }, [])
    ...
    <Container>
      ...
      <Description>
        {message || (
          <div>
            La página{' '}
            <PathName>{location.pathname.length > 32 ? `${location.pathname.substring(0, 29)}...` : location.pathname}</PathName>{' '}
            no fue encontrada.
          </div>
        )}
      </Description>
