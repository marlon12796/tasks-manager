 ...
 */
export const calculateDateDifference = (date: Date, lang: string = navigator.language || 'en-US'): string => {
  ...
  if (differenceDays <= 7) {
    ...
    return `${dayOfWeek} (${new Intl.RelativeTimeFormat(userLocale, {
      numeric: 'auto'
    }).format(differenceDays, 'day')})`
  }
  return new Intl.RelativeTimeFormat(userLocale, { numeric: 'auto' }).format(differenceDays, 'day')
}
