...
export const saveQRCode = (taskName: string) => {
  const modifiedTaskName = taskName.length > 32 ? `${taskName.substring(0, 32)}...` : taskName
  ...
  img.onload = () => {
    ...
    const textWidth = ctx.measureText(modifiedTaskName).width
    const centerX = (canvasWidth - textWidth) / 2
    const bottomY = canvasHeight - 35

    ctx.fillStyle = 'black'
    ctx.fillText(modifiedTaskName, centerX, bottomY)
    ...
    downloadLink.download = `QRCode${modifiedTaskName ? ` ${taskName}` : ''}.png`
