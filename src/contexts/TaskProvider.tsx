...
export const TaskProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [multipleSelectedTasks, setMultipleSelectedTasks] = useStorageState<UUID[]>([], 'selectedTasks', 'sessionStorage')
  ...
  const highlightMatchingText = (text: string): ReactNode => {
    ...
    return parts.map((part, index) =>
      part.toLowerCase() === search.toLowerCase() ? <HighlightedText key={index}>{part}</HighlightedText> : part
    )
  }
